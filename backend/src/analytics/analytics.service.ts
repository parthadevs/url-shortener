...
export class AnalyticsService {
  ...
  async getOverview(userId: string, timeRange: string = '30d') {
    ...
    const currentPeriodEarnings = clickEvents.reduce(
      (sum, event) => sum + Number(event.earnings),
      0,
    );
    const previousPeriodEarnings = previousClickEvents.reduce(
      (sum, event) => sum + Number(event.earnings),
      0,
    );
    ...
    const avgEarningsPerClick =
      currentPeriodClicks > 0 ? currentPeriodEarnings / currentPeriodClicks : 0;
    ...
    return {
      ...
      totalEarnings: totalStats._sum.earnings
        ? Number(totalStats._sum.earnings)
        : 0,
      clicksPercentChange,
  ...
  async getTopUrls(
    ...
    const topUrls = urls.map((url) => {
      const earnings = url.clickEvents.reduce(
        (sum, event) => sum + Number(event.earnings),
        0,
      );
      return {
